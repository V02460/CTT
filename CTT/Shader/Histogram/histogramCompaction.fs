...
void main() {
	...
	// one of four positions in the source texture relating to the calculated pixel
	ivec2 sourcePos = 2 * blockCorner + posInBlock;
	...
	for (delta.y = 0; delta.y < 2; delta.y++) {
		for (delta.x = 0; delta.x < 2; delta.x++) {
			ivec2 lookupPos = sourcePos + 16 * delta;
